...
def runAnalysis(input_file, output_file, file_type):
    # Endpoint URL
    endpoint = r"https://nayana-form-recognizer.cognitiveservices.azure.com/"
    # Subscription Key
    apim_key = "f4aa51d9e49e4ac29d3cce128565fba1"
    # Model ID
    model_id = "3d31e031-8000-4fdf-a44e-20a3e4d35d24"
    # API version
    API_version = "v2.1"

    post_url = endpoint + "/formrecognizer/%s/custom/models/%s/analyze" % (
...
def getArguments(argv):
    ...
    for opt, arg in opts:
        if opt == '-t':
            if arg not in ('application/pdf', 'image/jpeg', 'image/png',
...
def inferrType(input_file):
    ...
    if file_extension == '':
        print(
            'File extension could not be inferred from inputfile. Provide type as an argument.'
        )
        sys.exit()
...
def printCommandDescription(exit_status=0):
    ...
    print
    print(
        "If type option isn't provided, type will be inferred from file extension."
    )
